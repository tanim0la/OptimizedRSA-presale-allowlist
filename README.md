...
I was able to shave off `163` gas from each test when the function `verifySignature` is called. Here are the changes I made:
...
```
#
The variables `modPos` and `callDataSize` were replaced with `add(0xe0, add(sig.length, 0x20))` and `add(0x80, mul(sig.length, 2))` respectively in `extcodecopy` and `staticcall` opcodes because the variables were use just once.
...
```
#
I noticed the for loop started its check at position `0x80`, so I was able to skip the first two loops using the `or` opcode to compare each byte at positon `0x80` and `0xa0` then `gt` opcode to check if the result is greater than 0, if yes, the function revert, else, it means the values at position `0x80` and `0xa0` are zero.
...
```
#
The variable `decodedSig` was replaced with `mload(add(0x60, sig.length))` and the position `0x00` was replaced with `returndatasize` opcode since it cost only 2 gas.
...
- RSA 896 bit Metamorphic  (Gas: 26,877)
- RSA 960 bit Metamorphic  (Gas: 26,952)
- RSA 1024 bit Metamorphic (Gas: 27,148)
- RSA 2048 bit Metamorphic (Gas: 29,738) 


RSA 2048 bit gas can be beaten down to `29,562` gas if the counter `i` in the for loop is increment by 2, but doing this will skip some chunks to check which breaks the cryptography.
<hr>
